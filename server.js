...
app
  ...
  .use(bodyParser.json())
  .use(session({secret: 'asdlkfhaklwjhf123oi1n2lkn1u2039i12lk3ncpijp1o2j3'}))
  .use(express.static('./public'))
  .get('/api/league_table',function(req, res){
    Model.TeamModel.find({},'_id name totalPoints', function(err, teams){
        res.send({teams: teams.sort(function(a, b) {
  ...
  app.use('/api/myTeam', function(req, res){
    Model.TeamModel.findOne({_id: req.session.user.teamId}).exec(function(error, team){
      var playersPopQuery = [{path: 'team.squad', model: 'players'}, {path: 'team.eleven', model: 'players'}, {path: 'team.subs', model: 'players'}];
